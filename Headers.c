...
void rechercheDriverParMatricule(VOITURE* v, int nbv, int matriculerecherche) {
    ...
    if (!trouve) {
        printf("Aucune voiture avec la matricule %d n'a �t� trouv�e.\n", matriculerecherche);
    }
}
...
void recherchePersoParCIN (PERSONNEL* perso, int nbperso, int CIN){
    ...
    if (!trouve) {
        printf("Aucun personnel avec la CIN %d n'a �t� trouvee.\n", CIN);
    }
}
...
DRIVER lireUnDriverDuFichier(FILE* fp) {
    ...
    fread(&drv.a, sizeof(bool), 1, fp);     // Read a boolean for "abonn�"
...
void ecrireUnDriverDansFichier(FILE* fp, DRIVER drv) {
    ...
    fwrite(&drv.a, sizeof(bool), 1, fp);     // Write the "abonn�" boolean flag
