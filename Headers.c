...
void afficherMenuFinances(DRIVER* drv, int nc,DEPENSES* d,int nbrDepenses) {
    ...
    do {
        ...
        printf("*2  - Gestuoin les personels         \n");
            ...
            case 1:
                ...
                case 2:
                {
                   ...
                   if(indice!=-1){
                        ...
                        afficherMenuPersonnels(&(*(d+indice)).personnel,&(*(d+indice)).nbrperso,d,nbrDepenses);
...
void creerFichierPersonnel(FILE**fp)
{
...
void afficherFichierIndex(FILE* fi){
    ...
    while(1){
        ...
        printf("ttttttttttttttttt %d",nbr);
        if(!feof(fi)) break;
    }
}
...
void creerFichierVoiture(FILE**fp)
{
	*fp= fopen("voiture","wb+");
	if(!*fp) exit(-1);
}
...
VOITURE lireUnVoitureDuFichier(FILE*fp)
{
	VOITURE v;
...
void afficherFichierVoitures(FILE*fp)
{
	...
	while(1)
	{
		...
		if(feof(fp)) break;
		fseek(fp, -sizeof(int),1);
		v=lireUnVoitureDuFichier(fp);
		afficherVoiture(v);
	}
}
void creerFichierDepenses(FILE**fp)
{
...
void ecrireLesDepensesDansFichier(FILE*fp, DEPENSES d)
{
	...
	fwrite(&d.maintenance, sizeof(int[3]), 3, fp);
...
DEPENSES lireLesDepensesDuFichier(FILE*fp)
{
	...
	fread(d.maintenance, sizeof(int), 3, fp);

	fread (&d.nbrperso ,sizeof(int), 1 , fp);
	fread (&d.personnel, sizeof(PERSONNEL), 1, fp);
	return d;
}
    ...
    *driver=creerTabDriver(*NbrDriver);
    copierFichierDriverDansTableau(*FDrive,*FIDriver,*driver,*NbrDriver);
      for (int i = 0; i < *NbrDriver; i++) {
        // Using pointer arithmetic instead of array notation
        *( *driver + i ) = lireUnDriverDuFichier(*FIDriver);  // Load driver into the i-th position
    }
